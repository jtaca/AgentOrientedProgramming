			...
			if(bestTask.beenNegative) {
				...
				for(Task t: neverNegative) {
					...
					if (t.realUtility.compareTo(otherBestTask.realUtility) > 0 && 
							((!t.beenZero && otherBestTask.beenZero)||!t.beenZero && !otherBestTask.beenZero)) {
						otherBestTask = t;	
					}
...
//						}
						...
						if(result < epsilon) {//result == (float) 0){
							weight1 = i; weight2 = j;
							//minresult = result;
						}
						i+=(float)0.0001;
					...
					if(weight1 >weight2 && Math.abs(weight1-0.50) > epsilon) {
						bestTaskPair = new TaskPair(bestTask,otherBestTask,weight1,weight2);
					}else {
						bestTaskPair = new TaskPair(otherBestTask,bestTask,weight2,weight1);
					}
					...
					if(weight1 > weight2 && Math.abs(weight1-0.50) > epsilon){
						...
						System.out.println("{"+otherBestTask.action+"="+df.format(weight2)+","+bestTask.action+"="+df.format(weight1)+"}");
					}
...
//			}
			...
			//System.out.println(flexibleChoices);
	...
	public class Task implements Comparable<Task>{
		...
		public Boolean beenNegative, beenZero;
